...
--  The type CSR_Array is the argument for the *A CSRs (MEIEA, MEIPA,
--  MEIFA, MEIPRA).
--  INDEX is the interrupt number mod 16, the index in WINDOW is
--  interrupt number / 16.

--  Of eventual interest: MEICONTEXT (related to interrupt priority,
--  but complicated).
...
package body System.Interrupts.Xh3irq is
   ...
   --  MEINEXT always displays the next interrupt that should be
   --  handled, taking priority order into account.
   ...
   begin
      ...
      System.Machine_Code.Asm
        ("csrrs %0, meinext, 1",
         Outputs  => Xh3irq.MEINEXT_Register'Asm_Output
           ("=r", Next_Interrupt),
         Volatile => True);
      return Result : Next_Interrupt_Info (Valid => not Next_Interrupt.NOIRQ)
      do
   ...
   type Bit_Window_Content is array (0 .. 15) of Boolean
     with Pack, Size => 16;
   ...
   begin
      ...
      System.Machine_Code.Asm
        ("csrs meiea, %0",
         Inputs  => Xh3irq.MEIEA_Data'Asm_Input
           ("r", Data),
         Volatile => True);
   ...
   begin
      ...
      System.Machine_Code.Asm
        ("csrs meifa, %0",
         Inputs  => Xh3irq.MEIFA_Data'Asm_Input
           ("r", Data),
         Volatile => True);
   end Force_Interrupt;
   ...
   begin
      ...
      System.Machine_Code.Asm
        ("csrs meipa, %0",
         Inputs  => Xh3irq.MEIPA_Data'Asm_Input
           ("r", Data),
         Volatile => True);
   ...
   type Priority_Window_Content is array (0 .. 3) of Priority
     with Pack, Size => 16;
   ...
   procedure Read_Bit_Array is
      type Bit_Array is array (0 .. 15) of RP2350.Bit
        with Pack, Size => 16, Default_Component_Value => 0;
      Bit_Array_Array : array (0 .. 31) of Bit_Array
        with Size => 512;
   begin
